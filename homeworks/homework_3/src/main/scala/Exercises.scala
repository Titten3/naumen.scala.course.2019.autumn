object Exercises {
  ...
  class Numbers(val x: Seq[Int]) extends Iterable[Int]
  {
    override def iterator: Iterator[Int] = x.iterator
  }

  def sum1(x: Int, y: Int): Int = sumIntegers(Queue(x,y))
  def sum2(x: Int, y: Int): Int = sumIntegers(Seq(x,y))
  def sum3(x: Int, y: Int): Int = sumIntegers(new Numbers(Seq(x,y)))
