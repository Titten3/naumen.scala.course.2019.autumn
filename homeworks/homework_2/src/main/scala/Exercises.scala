object Exercises {
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, scalarOrCosBetween:(Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) =
    scalarOrCosBetween(leftVec0, leftVec1) + scalarOrCosBetween(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def massBalls (data :(Int, Double)):Double = {
      val (radius, volume) = data
      volume*(4.0/3)*java.lang.Math.PI*java.lang.Math.pow(radius,3)
    }
    def volumeBalls (data :(String, Double)):Double={
      val (radius, volume) = data
      volume
    }
    ballsArray.mapValues(massBalls).toSeq.sortBy(volumeBalls).unzip._1
  }
